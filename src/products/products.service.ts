...
export class ProductsService {
  ...
  create(createProductDto: CreateProductDto, vendedorId?: number) {
    const { vendedorId: dtoVendedorId, ...dto } = createProductDto;
    const vendedor = vendedorId ?? dtoVendedorId;
    const produto = this.repo.create({ ...dto, vendedorId: vendedor });
  ...
  async decrementStock(produtoId: number, quantidade: number): Promise<void> {
    const produto = await this.repo.findOneBy({ id: produtoId });
    if (produto) {
